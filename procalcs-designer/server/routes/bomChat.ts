...
import { routeTier } from "../modelRouting.js";
...
- drawing_annotations in the context are equipment-relevant notes the designer typed on the drawing canvas (e.g. "Dehumidifier") that have NO equipment record in the file — no model, no part number. When present, proactively tell the user the drawing mentions this equipment and offer propose_add_line once they supply the model; never invent a model yourself.
- register_preflight in the context reports how many register records in the design file are auto-sized vs user-set (Wrightsoft stores unset grille sizes as a literal 12x12 default). A large auto_count explains a lumped 12x12 grille line structurally. The TRUE sizes of auto records are NOT in the file — never estimate a size split; the fix is setting sizes in the Wrightsoft property sheet and rebuilding.
- PROVENANCE of duct names: run labels like "Supply Duct17" / "Return Duct20" are Wrightsoft's OWN internal names for the drawn duct runs, decoded from the .rup's drawing objects (CSDuctOb/CRDuctOb). They are NOT rows in the BOM report — the user cannot find them there. If asked where they come from, say exactly this: they're the drawing-canvas object names inside the design file (visible in Wrightsoft by selecting a drawn run), not BOM entries.
...
- Heat strips (elec strips): EXPERT-CONFIRMED policy (Richard, 2026-07-29) — there is NO standing rule. "1 per split system" is a general default, not fixed; some equipment/projects have none. The BOM shows what is placed in the design file; lines flagged "verify" have an ambiguous count the user should confirm. Do NOT offer to make heat-strip counts a standing rule, and do NOT treat a deviation as an error — ask the user what this project needs.
Domain notes: RHEA = Rheia (small-diameter duct system, rheiacomfort.com). BOMs historically exist only for Rheia projects; standard projects are the new territory. "RE" suffix files are revisions. Rheia jobs are handled by Wrightsoft's own Rheia plugin — this tool exists for conventional/standard projects, which is why derived Rheia sections are suppressed when a project's priced BOM carries a full conventional duct system.`;
...
router.post("/", async (req: Request, res: Response) => {
  ...
  // Prompt caching (cost lever). The first block already caches the
  // static instructions (helps across conversations). Add a second
  // breakpoint on the LAST system block so the whole prefix —
  // instructions + the ~120 KB BOM draft + attachments — is cached
  // too. It's byte-identical across the up-to-8 tool-loop turns and
  // across follow-up messages within the cache TTL, so those reads
  // cost ~10% of the input instead of re-sending ~120 KB each time.
  const lastSys = system[system.length - 1];
  if (lastSys && lastSys.type === "text") {
    lastSys.cache_control = { type: "ephemeral" };
  }
  ...
  // Which model this exchange runs on. The routing policy (opt-in via
  // CHAT_ROUTING_POLICY=tiered) may pick a tier from the request; when
  // it has no opinion the model layer falls back to CHAT_MODEL /
  // CHAT_MODEL_TIER / premium. Default behavior is unchanged (Opus).
  ...
  const lastUserText = typeof lastUserMsg?.content === "string"
    ...
    : Array.isArray(lastUserMsg?.content)
      ...
      : "";
  const chatModel = resolveModel(routeTier({
    userText: lastUserText,
    hasAttachments: attachments.some((a) => a.kind !== "snipe"),
    hasSnipes: attachments.some((a) => a.kind === "snipe"),
  }));

  // Cost observability — accumulate token usage across the tool loop so
  // one chat_message event carries the full spend of the exchange. The
  // price table + cost math live server-side (Python model_pricing);
  // here we log raw tokens + model, which stays correct if prices move.
  const usage = {
    input_tokens: 0,
    output_tokens: 0,
  ...
  try {
    ...
    // Loop exhausted (8 tool rounds) — still record the spend so these
    // (the most expensive) exchanges aren't invisible in cost reports.
    await logUsage(req, "chat_message", {
      snipes: attachments.filter((a) => a.kind === "snipe").length,
      attachments: attachments.filter((a) => a.kind !== "snipe").length,
      actions_proposed: actions.length,
      turns: messages.length,
      stopped_at_tool_limit: true,
      model: chatModel,
      ...usage,
    }, { client_id });
    res.json({
