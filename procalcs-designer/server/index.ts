...
import { requireRoleForWrites } from "./auth/authorize.js";
import { chatRateLimit, uploadRateLimit } from "./rateLimit.js";
...
//
// Authorization (security hardening): reads stay open to any
// authenticated user; MUTATIONS to admin-owned surfaces require the
// `admin` role. Audit-only until AUTHZ_ENFORCE=true (see authorize.ts) —
// so this logs would-be denials without changing behavior yet.
app.use("/api/client-profiles", requireAuth, requireRoleForWrites("admin"), clientProfilesRouter);
app.use("/api/dashboard", requireAuth, dashboardRouter);
app.use("/api/bom", requireAuth, uploadRateLimit, bomRouter);
app.use("/api/bom-runs", requireAuth, bomRunsRouter);
app.use("/api/sku-catalog", requireAuth, requireRoleForWrites("admin"), skuCatalogRouter);
...
app.use("/api/bom-chat", requireAuth, chatRateLimit, bomChatRouter);
