...
export interface BomResponse {
  ...
  run_id?: number;
  // Reviewer Workspace — surfaced from the bom_run row on rehydrate.
  reviewer_status?: string;   // one of REVIEWER_STATUSES
  reviewer_notes?: string | null;
  patch_ops?: PatchOp[];      // correction history (audit trail)
  source_rup_filename?: string;
}

// One recorded correction on a run (the audit trail entry).
export interface PatchOp {
  ...
  author?: string;
  at?: string;
  snipe_ref?: string | null;
}

// Settable review statuses (ReviewerStatus type is defined further down
// alongside the bom-runs hooks). Array form for UI iteration.
export const REVIEWER_STATUSES = ["unset", "good", "needs_fix", "blocked"] as const;
